...
ALTER TABLE public.saved_job_searches
    DROP CONSTRAINT IF EXISTS saved_job_searches_result_limit_check;

ALTER TABLE public.saved_job_searches
    ADD CONSTRAINT saved_job_searches_result_limit_check
        CHECK (result_limit >= 1 AND result_limit <= 200);
...
ALTER TABLE public.company_watchlists
    DROP CONSTRAINT IF EXISTS company_watchlists_source_hint_check;

ALTER TABLE public.company_watchlists
    ADD CONSTRAINT company_watchlists_source_hint_check
        CHECK (
            source_hint IN (
                'auto',
                'greenhouse',
